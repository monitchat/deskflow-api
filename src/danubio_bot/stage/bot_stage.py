...
from danubio_bot.common import validate_cpf, format_cpf
from danubio_bot.client import support, danubio
from danubio_bot.config import (
    ID_DEPARTAMENTO_CREDIARIO,
    ID_DEPARTAMENTO_ASSISTENCIA_TECNICA,
)
from danubio_bot.stage.bot_stage_abstract import BotStage
from danubio_bot.stage.buttons import ASK_YES_NO_BUTTONS, HOW_CAN_WE_HELP_NON_PARTNER_BUTTONS
...
class AskStartMenuStage(BotStage):
    ...
    def handle_input(self, msisdn: str, text: str = None) -> list:
        ...
        partner = danubio.get_partner(phone=msisdn).get('responseBody', {}).get('records', {}).get('record', None)
...
class BotSentProdutosPendenteEntregaStage(BotStage):
    ...
    def handle_input(self, msisdn: str, text: str = ""):
        ...
        customer_orders = danubio.get_products(codigo=codparc).get('responseBody', {}).get('records', {}).get('record', None)
...
class BotSentServicosPendenteStage(BotStage):
    ...
    def handle_input(self, msisdn: str, text: str = ""):
        ...
        codparc = self.get_context_value(msisdn=msisdn, property="codparc")
        log.info(f"codparc: {codparc}")
        customer_services = danubio.get_services(codigo=codparc).get('responseBody', {}).get('records', {}).get('record', None)
...
class BotReceiveCpfClienteStage(BotStage):
    ...
    def handle_input(self, msisdn: str, text: str) -> list:
        ...
        codparc = self.get_context_value(msisdn=msisdn, property="codparc")
        customers = danubio.get_client(cpf=cpf, codparc=codparc).get('responseBody', {})
...
class BotReceiveCpfNonPartnerStage(BotStage):
    ...
    def handle_input(self, msisdn: str, text: str) -> list:
        ...
        data = danubio.get_client(cpf=cpf, codparc=codparc).get('responseBody', {})
...
class ReceiveReceberAtualizacoesEntregaStage(BotStage):
    ...
    def handle_input(self, msisdn: str, text: str) -> list:
        if text.lower() in ["sim", "yes", "si"]:
            codparc = self.get_context_value(msisdn=msisdn, property="codparc")
            resp = danubio.update_parceiro(codparc=codparc, alert="S")
            log.info(f"Atualização de parceiro: {resp}")
            self.set_context(msisdn=msisdn, data={"receber_atualizacoes": True})
        else:
            resp = danubio.update_parceiro(codparc=codparc, alert="N")
            log.info(f"Atualização de parceiro: {resp}")
