...
from danubio_bot.config import DANUBIO_API_BASE_URL, DANUBIO_PASSWORD, DANUBIO_USER
...
def create_session():
    ...
    url = f"{DANUBIO_API_BASE_URL}?serviceName=MobileLoginSP.login&outputType=json"
    
    data = {
        "serviceName": "MobileLoginSP.login",
        "requestBody": {
            "NOMUSU": {"$": DANUBIO_USER},
            "INTERNO": {"$": DANUBIO_PASSWORD},
            "KEEPCONNECTED": {"$": "S"}
        }
    }
...
def update_parceiro(codparc: str, alert: str):
    url = f"{DANUBIO_API_BASE_URL}?serviceName=DatasetSP.save&outputType=json"
...
def get_partner(phone: str):
    ...
    try:
        resp = requests.get(
            f"{DANUBIO_API_BASE_URL}?serviceName=CRUDServiceProvider.loadView&outputType=json",
